...
export default function ScenarioCard({ 
  ...
  return (
    <div className="space-y-4 md:space-y-6">
      {/* Scenario Prompt - Lilac, more right padding (stimulus) */}
      <div className="bg-lilac-200 border-lilac-400 rounded-lg border p-4 md:p-6 pl-4 pr-8 text-left w-full"> 
        <div className="text-base md:text-lg font-medium text-lilac-900 leading-tight">
          {prompt}
        </div>
      </div>

      {/* Response Options - Lighter lilac, more left padding (response) */}
      <div className="space-y-3 md:space-y-4">
        {responses.map((response) => (
          <div 
            key={response.id} 
            onClick={() => handleResponseClick(response.id)}
            className="relative bg-lilac-100 border-lilac-300 rounded-lg border p-4 md:p-6 pl-8 pr-12 md:pr-16 
